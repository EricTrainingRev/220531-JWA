...
-- that already exists: 
-- primary key ensures that id is unique and can't be null
create table if not exists pet (id serial primary key, pet_name varchar(50), species varchar(50), color varchar(50), pet_age integer, legs integer, bree varchar(50));
...
alter table pet drop column bree;
-- add correctly spelled column
alter table pet add column breeed varchar(50);
-- we made another typo, so now we can edit it directly:
alter table pet rename column breeed to breed;
